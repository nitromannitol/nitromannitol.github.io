...
\date{\small Last Updated: 2 September 2026}
...
produced entirely by ChatGPT 5.6 Sol and Claude Fable 5.1, with minimal human intervention.
\par\smallskip
\textbf{Warning:} This manuscript has not yet undergone extensive independent proofreading.
\end{center}
...
\]
The tempting picture is to follow an open path from $o$ until it first reaches $A$ and then continue
toward $b$. The difficulty is that the vertex of $A$ selected by the first path is chosen
...
of the critical-percolation problem~\cite{KN}.
...
\centering
\begin{tikzpicture}[
  vertex/.style={circle,draw,fill=white,inner sep=1.5pt,minimum size=6mm},
  open/.style={line width=1.1pt}
]
  \node[vertex] (o) at (0,0) {$o$};
  \node[vertex] (u) at (1.2,0) {};
  \node[vertex] (x1) at (2.7,1.55) {$x_1$};
  \node[vertex] (x2) at (2.7,0.55) {$x_2$};
  \node[vertex] (x3) at (2.7,-0.55) {$x_3$};
  \draw[open] (o)--(u)--(x2);
  \draw[open] (u)--(x3);
  \begin{scope}[on background layer]
    \node[draw,rounded corners,dashed,fit=(x1)(x2)(x3),inner sep=5pt,
          label=above:$T$] {};
    \node[draw,ellipse,fit=(o)(u)(x2)(x3),inner sep=6pt,
          label=below left:$C_o$] {};
  \end{scope}
  \node[anchor=west] at (3.55,0.45)
    {$r(x_1)<r(x_2)<r(x_3)$};
  \node[anchor=west] at (3.55,-0.15)
    {$x_1\notin C_o,\quad x_2,x_3\in C_o$};
\end{tikzpicture}
\caption{The rank $r$ is fixed before the configuration is sampled. Here
$r(x_1)<r(x_2)<r(x_3)$ and $T\cap C_o=\{x_2,x_3\}$, so $x_2$ is the
smallest-ranked vertex of $T\cap C_o$ and $J_{x_2}$ occurs. The schematic layout carries
no metric meaning: this is not a first-hit statement along a path.}
...
We now state the model and the nine Kozma--Nitzan problems in full, making the paper
independent of the notation of~\cite{KN}.
...
open with probability $p_e$. An edge with $p_e=0$ may be regarded as absent. Write $\PP$
and $\EE$ for the resulting probability and expectation.
...
with at least one endpoint in $C_x$. For an edge set $K$, let $\spn K$ be the set of
endpoints of edges in $K$, so that $C_x=\{x\}\cup\spn\eC_x$.
...
numbers is increasing if $F(K)\le F(L)$ whenever $K\subseteq L$. Every set in this note is
finite because $V$ is finite, and $A$ is always a nonempty subset of $V$.

Kozma and Nitzan write $\PP(0\con b)$ where we write $\PP(o\con b)$. They call a function
$f$ of a vertex and a configuration a monotone cluster property if $f(v,\omega)$ depends
only on the open cluster of $v$ and is increasing under inclusion of that cluster, and if
$f(v,\omega)=f(w,\omega)$ whenever the edge $\{v,w\}$ is open. Their five conjectures and four questions are the
...
$(c_x)_{x\in A}$, depending on $A$, $o$, and the edge probabilities but not on $b$, such
that $c_x\geq0$, $\sum_{x\in A}c_x=1$, and, for every $b\in V$,
\[
...
\begin{question}[Kozma and Nitzan, page 36]\label{q8}
...
We use two families of correlation inequalities. We state them in the precise forms used
in Sections~\ref{sec:gen} and~\ref{sec:c6}, then record the conditional identities needed
when several clusters are explored at once.
...
\end{equation}
where $B_1=\{s\}\cup Y$ and $B_{j+1}=B_j\cup\{z_j\}$ for $1\le j\le\ell$. For a real
function $q$ on $V$ put $q^0=q$ and
\begin{equation}\label{eq:levels}
 q^j(u)=q^{j-1}(u)-\pi_j(u)\,q^{j-1}(z_j)\quad\text{for } 1\le j\le\ell,\qquad
...
$g(K)=\one\{v\in\{s\}\cup\spn K\}$, the displayed inequality is equivalent to Harris's
...
\cite[Lemma~2.4]{BHK}. Condition on $\eC_Y$. On the event $D=\{s\ncon Y\}$, the vertex set
$C_Y=Y\cup\spn\eC_Y$ is disjoint from $C_s$, every edge with exactly one endpoint in
...
Write $\bar g$ for the expectation of $g(\eC_s)$ under the product measure in which every
edge meeting $C_Y$ is closed and every remaining edge keeps its original probability. Then the random
variable
\begin{equation}\label{eq:resid}
 \Theta=\one_D\bigl(g(\eC_s)-\bar g\bigr)
\end{equation}
has $\EE[\Theta\mid\eC_Y]=0$. For a set $K$ of vertices disjoint from $\{s\}\cup Y$, let
...
$W$ of vertices write $\PP_W$, $\EE_W$ and $\Cov_W$ for the probability, the expectation
and the covariance in the percolation measure in which every edge with an endpoint outside
...
cluster of $o$. We prove it while requiring that cluster to miss a fixed set $Y$.
...
$m_x\le m_y$ whenever $r(x)<r(y)$. Such an $r$ exists: order $T$ by the values $m_x$ and
...
\begin{equation}\label{eq:minform}
 ...
 m_x\Bigr)\Bigr]\,,
\end{equation}
which does not refer to $r$. If $T\cap C_o=\varnothing$, the indicator in
...
$Y\cup T\cup\{o,v\}$. Let $\Marg$ be the functional \eqref{eq:levels} built from the
constants \eqref{eq:constants} with $Y$ replaced by $Y\cup T$ at every step. We start by
...
$T_0=T\setminus\{k\}$. Abbreviate
\[
 P=\PP(k\ncon Y\cup T_0),\qquad P(u)=\PP(k\ncon Y\cup T_0,\ u\con k),
\]
\[
 I=\EE\bigl[F(C_k);\,k\ncon Y\cup T_0\bigr],\qquad
...
\begin{equation}\label{eq:peel}
 \Lambda_u(T)=\Lambda_u(T_0)+I(u)-P(u)\,m_k\,.
\end{equation}

Write $\Cov^{\ast}(u)$ for $P\,I(u)-I\,P(u)$. This is the unnormalized conditional covariance in
Theorem~\ref{thm:csh} without denominators, for the vertex $k$, the set $Y\cup T_0$ and the
increasing function $\eC\mapsto F(\{k\}\cup\spn\eC)$. Put $\gamma_k=m_kP-I$. Expanding
both sides gives
\begin{equation}\label{eq:covid}
 P\bigl(I(u)-P(u)\,m_k\bigr)=\Cov^{\ast}(u)-\gamma_k\,P(u)\,.
\end{equation}
...
$\Marg[\Cov^{\ast}]\geq0$. To prove $\Marg[P]\geq0$, set
$h(K)=\one\{K\neq\varnothing\}$ and
\[
 q_0=\PP(k\ncon Y\cup T_0,\eC_k=\varnothing).
\]
For every vertex $u$ at which $\Marg$ evaluates its argument, $u\neq k$. Hence the
restricted expectations corresponding to $h$ are $I_h(u)=P(u)$ and $I_h=P-q_0$, so the
unnormalized conditional covariance is $q_0P(u)$. A second application of
Theorem~\ref{thm:csh} gives $q_0\Marg[P]\geq0$. Closing every edge incident to $k$ has
positive probability, so $q_0>0$ and $\Marg[P]\geq0$.
...
Now apply $\Marg$ to \eqref{eq:peel}, multiply by $P>0$ and use \eqref{eq:covid}:
\[
 P\,\Marg\bigl[\Lambda_\cdot(T)\bigr]=P\,\Marg\bigl[\Lambda_\cdot(T_0)\bigr]
 +\Marg[\Cov^{\ast}]-\gamma_k\,\Marg[P]\,.
\]
The middle term is nonnegative. Replace $\gamma_k$ by the larger $\Lambda_k(T_0)$, which is
legitimate because $\Marg[P]\ge0$, and write $P(u)=P\,\pi_k(u)$ with
$\pi_k(u)=\PP(u\in C_k\mid k\ncon Y\cup T_0)$. The right side is then at least $P$ times the
...
value is nonnegative by the induction hypothesis. Dividing by $P$ gives \eqref{eq:s5d}.

We now take $\ell=0$ in \eqref{eq:s5d}. The conclusion is $\Lambda_o(T)\ge\tau\Lambda_v(T)$
...
We prove by induction on the cardinality of $T$ that $\Lambda_o(T)\ge0$ for every vertex
...
$T_0=T\setminus\{k\}$, and keep the abbreviations $P$, $P(u)$, $I$ and $I(u)$ of the proof
of Proposition~\ref{prop:s5}. The identity \eqref{eq:peel} with $u=o$ reads
$\Lambda_o(T)=\Lambda_o(T_0)+I(o)-m_kP(o)$.

Apply \eqref{eq:bhk1} with $s=k$, with $X=Y\cup T_0$ and with the two increasing functions
$\eC\mapsto F(\{k\}\cup\spn\eC)$ and the indicator of the event that $o$ lies in the cluster
of $k$. The result is $P(o)\,I\le P\,I(o)$, which rearranges to
\[
 P\bigl(m_kP(o)-I(o)\bigr)\leq P(o)\bigl(m_kP-I\bigr)=P(o)\,\gamma_k\,.
\]
By \eqref{eq:kappa} and $P(o)\ge0$ the right side is at most $P(o)\Lambda_k(T_0)$, and by
Proposition~\ref{prop:s5} with $v=k$ that is at most $P\,\Lambda_o(T_0)$. Hence
$P\,\Lambda_o(T)\ge0$. If $P>0$, then $\Lambda_o(T)\geq0$. If $P=0$, then $P(o)=0$ and
$I(o)=0$, so $\Lambda_o(T)=\Lambda_o(T_0)$, which is nonnegative by the induction
hypothesis.
\end{proof}
...
Theorem~\ref{thm:gen} orders vertices using expectations conditioned on avoiding a fixed
set. The conjectures instead choose a vertex from unconditional expectations and only afterward impose the
event $\{o\con A\}$. We now bridge that difference and then deduce Conjectures~1--4 and
Question~7.

The obstacle is that the numbers $m_x$ of \eqref{eq:condmean} are conditional expectations,
whereas Conjecture~\ref{c4} compares unconditional ones. The following function removes the
difference. For a vertex set $K$, let $\EE_K^0$ denote expectation under the product
measure obtained by closing every edge incident to $K$ and retaining the original
probability of every other edge. For a vertex $a$ and an increasing $F$, define
\[
 \Phi_a(K)=F(K)-\EE_K^0[F(C_a)].
\]
...
$\EE_K^0[F(C_a)]$, so $\Phi_a$ is increasing.
...
The lemma turns unconditional minimality of $a$ into nonnegative conditional expectations. We can
therefore apply Theorem~\ref{thm:gen} with $Y=\{a\}$.

\begin{theorem}\label{thm:fixedmin}
Let $A$ be a nonempty finite set of vertices, let $a\in A$, let $o$ be a vertex, and let
$F$ be an increasing function of subsets of $V$ with $\EE[F(C_a)]\le\EE[F(C_x)]$ for every
$x\in A$. Then
\[
...
$(\EE[F(C_x)]-\EE[F(C_a)])/\PP(x\ncon a)\ge0$. The sum subtracted in \eqref{eq:surplus} is
therefore nonnegative, and Theorem~\ref{thm:gen} gives
$\EE[\Phi_a(C_o);\,o\ncon a,\ o\con T]\ge0$. The event $\{o\con T\}$ is determined by
...
a\}\cap\{o\con T\}$ only by configurations in which $o$ misses $a$ and reaches at least one
vertex $x$ of $A\setminus\{a\}$ outside $T$. Such a configuration lies in the null event
...
Let $f:V\times\Omega\to\mathbb R$. Assume that for every $v\in V$ and configurations
...
S_1=C_v(\omega_{S_1})$, so $F(S)\le F(S_1)$. Extend $F$ to the empty set by a value at most
...
We can now translate the fixed-minimizer theorem back into the cluster-property language
...
Theorem~\ref{thm:fixedmin} gives $\PP(a\con b,o\con A)\le\PP(o\con
...
the Harris inequality gives $\PP(o\con A)\PP(x\con b)\le\PP(x\con b,o\con A)$ for every
...
\]
For Conjecture~\ref{c3} take $\delta=\varepsilon/2$. If $\varepsilon\ge2$, then
$1-\varepsilon\le-1<\PP(o\con b)$ and there is nothing to prove. Otherwise
$1-\varepsilon/2$ is positive, and Conjecture~\ref{c1} gives $\PP(o\con
b)>(1-\varepsilon/2)^2=1-\varepsilon+\varepsilon^2/4\ge1-\varepsilon$.
\end{proof}

The fixed-minimizer theorem allows the minimizing vertex of $A$ to depend on the target $b$.
...
Let $A$ be a nonempty finite set of vertices, let $o$ be a vertex and let $y_b\ge0$ for
...
\[
 ...
 \leq \PP(o\con b,\ o\con A)\leq \PP(o\con b).
\]
In particular Question~\ref{q5} has an affirmative answer.
\end{theorem}
...
w\rangle<t\le\langle y,\mathbf z\rangle$ for every $\mathbf z\in\mathcal N$. Fix $b\in V$ and fix a point
...
N$, so $\langle y,\mathbf z\rangle+n\,y_b\ge t$ for every $n$, which forces $y_b\ge0$.

Lemma~\ref{lem:q5dual} therefore gives a vertex $a\in A$ with $\langle
y,M(a,\cdot)\rangle\le\langle y,\mathbf w\rangle$. But $M(a,\cdot)$ lies in $\mathcal N$,
so separation gives $\langle y,M(a,\cdot)\rangle\ge t>\langle y,\mathbf w\rangle$, a
contradiction. Hence $\mathbf w$ lies in $\mathcal N$: there are $c_x\ge0$ with $\sum_{x\in
...
$\EE[F(C_a)]\le\EE[F(C_x)]$ for every $x\in A$. Then
\begin{equation}\label{eq:fm2}
 \EE\bigl[F(C_R)-F(C_a);\ R\ncon a,\ R\con A\setminus\{a\}\bigr]\geq 0\,.
\end{equation}
\end{theorem}

We first deduce Conjecture~\ref{c6} from the theorem. We then prove the theorem in nine
steps. The first four compare a set $R$ with a single vertex after a cluster is exposed.
Steps~5--7 extend the conditional covariance inequality from one starting vertex to $R$.
The last two steps repeat the induction over $A$ and remove the conditioning.
...
clusters of the endpoints is unchanged when $e$ is opened, and the expectation of any
...
\begin{tikzpicture}[
  ...
  \node at (1,1.65) {$e$ closed};
  \node[vertex] (u0) at (-1.1,0.7) {$u$};
  \node[vertex] (v0) at (0,0) {$v$};
  \node[vertex] (w0) at (2,0) {$w$};
  \node[vertex] (z0) at (3.1,0.7) {$z$};
  \draw[open] (u0)--(v0);
  \draw[dashed] (v0)--node[above] {$e$} (w0);
  \draw[open] (w0)--(z0);
  \draw (0.92,0.16)--(1.08,-0.16);
  \begin{scope}[on background layer]
    \node[draw,ellipse,fit=(u0)(v0),inner sep=5pt,label=below:$C_v$] {};
    \node[draw,ellipse,fit=(w0)(z0),inner sep=5pt,label=below:$C_w$] {};
  \end{scope}

  \draw[-{Latex[length=2.5mm]},thick] (3.65,0.05)--node[above]{open $e$}(4.85,0.05);

  \node at (7.5,1.65) {$e$ open};
  \node[vertex] (u1) at (5.4,0.7) {$u$};
  \node[vertex] (v1) at (6.5,0) {$v$};
  \node[vertex] (w1) at (8.5,0) {$w$};
  \node[vertex] (z1) at (9.6,0.7) {$z$};
  \draw[open] (u1)--(v1);
  \draw[open] (v1)--node[above] {$e$} (w1);
  \draw[open] (w1)--(z1);
  \begin{scope}[on background layer]
    \node[draw,ellipse,fit=(u1)(v1)(w1)(z1),inner sep=6pt,
          label=below:{$C_v=C_w=C_{\{v,w\}}$}] {};
  \end{scope}
\end{tikzpicture}
\caption{The configurations differ only in the state of $e=\{v,w\}$. In the pictured
case, opening $e$ merges $C_v$ and $C_w$, but in every case it leaves their vertex union
$C_{\{v,w\}}$ unchanged. On $\{\{v,w\}\ncon a\}$, the cluster $C_a$ is also unchanged.
The layout is schematic.}
...
$v$ or $w$ to $u$. If the path does not use $e$, it lies in $\omega$. If it uses $e$,
deleting $e$ leaves a final segment in $\omega$ from one endpoint of $e$ to $u$. Thus
$u\in C_v(\omega)\cup C_w(\omega)$, proving the equality of cluster unions. The two event
identities follow. If $\{v,w\}\ncon a$ in $\omega$, no path from $a$ in $\omega$ reaches
either endpoint of $e$, so opening $e$ cannot change $C_a$.
\end{proof}
...
\begin{equation}\label{eq:guardev}
 H(R;X,B)=\{R\ncon B\}\cap\{R\con X\}\,,
\end{equation}
the event that $C_R$ is disjoint from $B$ and intersects $X$. The event $\{R\con s\}$ does not imply
...
$C_R$ and $C_R$ misses $Y$, so $C_s\subseteq C_R$ misses $Y$.
...
$q^j(R)=q^{j-1}(R)-\pi_j(R)q^{j-1}(\{z_j\})$ for $1\le j\le\ell$, and finally, for a
prescribed set $R_0$,
\begin{equation}\label{eq:margin}
 \Marg[q]=q^\ell(R_0)-\tau(R_0)\,q^\ell(\{v\})\,.
\end{equation}
At a singleton $R_0$ the numbers \eqref{eq:profile} are the constants
\eqref{eq:constants}, and $\Marg[\Gamma]\ge0$ is Theorem~\ref{thm:csh}. The goal of the
first seven steps is the same conclusion for a set $R_0$ with at least two elements.

\medskip\noindent\textbf{Step 1: Linearity.}
There exist real coefficients $(c_u)_{u\in V}$, depending only on
$z_1,\dots,z_\ell$, $v$, and $R_0$, such that
\[
...
$R\subseteq U$ be disjoint from $X\cup\{v\}$. For a set $K$ of vertices put
...
$W$. We abbreviate $\phi_U$ and $\chi_U$ to $\phi$ and $\chi$.
...
\]
Write $\phi=\phi_U$ and $\chi=\chi_U$. Assume that, for all $W\subseteq U$ containing
$s$ and $v$ and all $Q\subseteq W$,
\begin{equation}\label{eq:stopped-general}
 \phi_W(Q)\,\PP_W(v\ncon X)
 \leq \PP_W(v\ncon X\cup Q)\,\eta(W),
\end{equation}
and assume that $Q\mapsto\phi_W(Q)$ is decreasing. Then
\begin{equation}\label{eq:twosource}
 \PP_U\bigl(R\ncon X\cup N,\ R\con v\bigr)\,\phi(N_1)
 \leq \PP_U\bigl(v\ncon X\cup N\cup N_1\bigr)\,\chi(N\cap N_1)\,.
\end{equation}
\end{lemma}
...
\[
 ...
 \leq \PP_U(v\ncon X\cup N_1)\eta(U)\,.
\]
The exchange inequality \eqref{eq:exch1} gives
\[
...
$\chi(\varnothing)=\varrho(U)\eta(U)$ to obtain \eqref{eq:twosource}. If
$\PP_U(v\ncon X)=0$, then
\[
 \PP_U(R\ncon X\cup N,R\con v)
 \leq \PP_U(v\ncon X\cup N)
...
\[
 ...
 \chi_U(Z)&=\sum_{S\subseteq U_0}\mu_Z(S)K(S).
 \end{aligned}
\]
...
\eqref{eq:etadef}. Then $\eta_v(U)\ge0$ and
\begin{equation}\label{eq:fourpoint}
 \varrho(U)\,\eta_v(U)\leq \eta_R(U)\,.
\end{equation}
Write $J$ for $\{R\ncon X\}\cap\{R\con v\}$. The union of $\{R\con X\}$ and $J$ is
increasing, so Harris gives $\eta_R\ge\EE_U[g(\eC_s)]\PP_U(J)-\EE_U[g(\eC_s);J]$. The
...
$S_2=\{v\}$ gives $\PP_U(v\ncon X)\EE_U[g(\eC_s);J]\le\PP_U(J)\EE_U[g(\eC_s);v\ncon X]$,
...
$g(K)=\Psi(\{z\}\cup\spn K)$. Expanding $\Gamma_{z,B}$, then using the
...
\end{equation}
and hence, dropping $\Delta(R)\ge0$,
...
increasing function
\[
 K\longmapsto\Psi_q(\{z_j\}\cup\spn K).
\]
...
\[
 ...
 \one\!\left\{
 R\cap(\{s\}\cup\spn K_s)\neq\varnothing,\
 R\cap(Y\cup\spn K_Y)=\varnothing
 \right\}\right].
\]
...
\]
Thus $\mathcal C(q)$ is the unnormalized covariance under $\PP(\,\cdot\mid D)$. By
...
conditional expectation given $\eC_s$; it determines $\mathcal K$ without any choice of
versions on null states.

For completeness, define the reverse conditional-covariance term by
\[
 \mathcal W_2(q)=\PP(D)\,
 \EE_\nu\!\left[
...
\begin{equation}\label{eq:two-cluster-step}
 \mathcal C(q)-\mathcal C(\mathcal Kq)
...
\[
 \mathcal C(\mathcal Kq)\leq\mathcal C(q).
\]

It remains to show that this monotonicity forces $\mathcal C(q)\geq0$. For a function on a
...
\]
If $H_0=\max_{K,L}|h(K,L)|$, then
\[
 |\mathcal C(\mathcal K^nq)|
 \leq2\PP(D)^2H_0\,\operatorname{osc}(\mathcal K^nq)
 \leq2H_0\beta^n\operatorname{osc}(q).
\]
Iteration gives $\mathcal C(\mathcal K^nq)\leq\mathcal C(q)$, while the displayed bound
tends to zero. Hence $\mathcal C(q)\geq0$.
...
\begin{proof}
If $R$ is a singleton, the conclusion is Theorem~\ref{thm:gen}. Assume first that $R$ has
...
For each $S\subseteq T$, let $\mathsf P(S)$ assert that
\eqref{eq:source-margin}, with $S$ in place of $T$, holds for every nonempty $R$ with
...
$r(k)$, and write
$S_0=S\setminus\{k\}$. To lighten notation, write $T=S$ and $T_0=S_0$ for the remainder
of this induction step. Then $m_x\leq m_k$ for every $x\in T_0$.
Write
\[
 E_k=\{k\ncon Y\cup T_0\},\qquad
 G_k(Q)=H(Q;\{k\},Y\cup T_0),\qquad d_k=\PP(E_k),
\]
and define
\[
...
Adding the vertex $k$, whose value $r(k)$ is maximal, to $T_0$ gives
\[
 \Lambda_Q(T)=\Lambda_Q(T_0)
   +\EE[F(C_Q)-m_k;\,G_k(Q)]\,.
\]
On $G_k(Q)$ we have $C_k\subseteq C_Q$. Monotonicity of $F$ therefore gives the lower
bound
\[
 \Lambda_Q(T)\geq\Lambda_Q(T_0)+K_k(Q),
\]
with equality whenever $Q$ is a singleton. Step~1 now gives
\begin{equation}\label{eq:source-peel-margin}
 \Marg[\Lambda_\cdot(T)]
 \geq \Marg[\Lambda_\cdot(T_0)+K_k]\,.
\end{equation}

Let $g_k(K)=F(\{k\}\cup\spn K)$ and define
\[
 \Gamma_k(Q)=d_k\EE[g_k(\eC_k);G_k(Q)]
 -\EE[g_k(\eC_k);E_k]\PP(G_k(Q)).
\]
This is \eqref{eq:gamma} with starting vertex $k$ and avoided set $Y\cup T_0$. Direct
...
Partitioning $\{k\ncon Y\}$ according to the first vertex of $T_0$ connected to $k$ gives
\begin{equation}\label{eq:source-repricing}
 \Lambda_{\{k\}}(T_0)-\kappa_k
 =\sum_{x\in T_0}\PP(J_x(\{k\}))(m_k-m_x)\geq0\,.
\end{equation}
The induction hypothesis for $T_0$, with $k$ inserted before
$z_1,\ldots,z_\ell$ in the auxiliary list, is
\[
 \Marg[\Lambda_\cdot(T_0)]
 -\Lambda_{\{k\}}(T_0)\Marg[c_k]\geq0\,.
\]
...
\[
\begin{aligned}
 d_k\Marg[\Lambda_\cdot(T)]
 &\geq d_k\Marg[\Lambda_\cdot(T_0)+K_k]\\
 &=d_k\Marg[\Lambda_\cdot(T_0)]
   +\Marg[\Gamma_k]-\kappa_kd_k\Marg[c_k]\\
 &\geq d_k\bigl(\Marg[\Lambda_\cdot(T_0)]
   -\Lambda_{\{k\}}(T_0)\Marg[c_k]\bigr)
   +\Marg[\Gamma_k]\geq0.
\end{aligned}
\]
Since $d_k>0$, this proves \eqref{eq:source-margin}.

Apply \eqref{eq:source-margin} to $T_0$, with no auxiliary vertices and with comparison
vertex $k$. The definition of $\Marg$ gives
\begin{equation}\label{eq:pairtransfer}
 \PP\bigl(G_k(R)\bigr)\Lambda_{\{k\}}(T_0)
 \leq d_k\Lambda_R(T_0)\,.
\end{equation}
...
\begin{equation}\label{eq:source-surplus-split}
...
\begin{equation}\label{eq:source-top-vertex}
...
$\eC\mapsto\Phi_a(\{s\}\cup\spn\eC)$ and $\eC\mapsto\alpha_{R,\{a\}}(\{s\}\cup\spn\eC)$.
Lemma~\ref{lem:proj}(ii) gives
\[
 \EE[\Phi_a(C_s);s\ncon a]
 =\EE[F(C_s)]-\EE[F(C_a)]\geq0.
\]
Since the other factor in \eqref{eq:bhk1} is nonnegative, the preceding identity gives
$\EE[\Phi_a(C_s);R\ncon a]\geq0$. On $\{R\ncon a\}$, the inclusion
...
$(\EE[F(C_x)]-\EE[F(C_a)])/\PP(x\ncon a)\ge0$, so $\EE[\Phi_a(C_R);R\ncon a,R\con
T_1]\ge0$; and \eqref{eq:projset} converts this into \eqref{eq:fm2} with $T_1$ in place of
...
\begin{tikzpicture}[
  vertex/.style={circle,draw,fill=white,inner sep=1.5pt,minimum size=6mm},
  chosen/.style={vertex,fill=black!15},
  open/.style={line width=1.1pt}
]
  \node at (-1.6,1.75) {original configuration};
  \node[chosen] (o0) at (-3.2,0) {$o$};
  \node[chosen] (u10) at (-1.6,1) {$u_1$};
  \node[vertex] (u20) at (-1.35,0) {$u_2$};
  \node[chosen] (u30) at (-1.6,-1) {$u_3$};
  \node[vertex] (p10) at (0,1.25) {};
  \node[vertex] (p30) at (0,-1.25) {};
  \draw[open] (o0)--(u10);
  \draw[dashed] (o0)--(u20);
  \draw[open] (o0)--(u30);
  \draw[open] (u10)--(p10);
  ...
  \draw[-{Latex[length=2.5mm]},thick] (0.55,0)--node[above,align=center]
    {condition on $\zeta$\\and close incident edges}(3.05,0);

  \node at (5.1,1.75) {law $\PP_\star$};
  \node[chosen] (o1) at (3.7,0) {$o$};
  \node[chosen] (u11) at (5.3,1) {$u_1$};
  \node[chosen] (u31) at (5.3,-1) {$u_3$};
  \node[vertex] (p11) at (6.9,1.25) {};
  \node[vertex] (p31) at (6.9,-1.25) {};
  \draw[open] (u11)--(p11);
  \draw[open] (u31)--(p31);
  \node[anchor=west] at (3.45,-1.85)
    {$R_\zeta=\{o,u_1,u_3\}$};
  \begin{scope}[on background layer]
    \node[draw,ellipse,dashed,fit=(u11)(p11),inner sep=5pt] {};
    \node[draw,ellipse,dashed,fit=(u31)(p31),inner sep=5pt] {};
  \end{scope}
\end{tikzpicture}
\caption{Fix the complete state $\zeta$ of the edges incident to $o$. Record the endpoints
of its open edges in $R_\zeta$, then delete every edge incident to $o$. For the same
nonincident-edge configuration, $C_o$ in the original graph is exactly the union
$C_{R_\zeta}$ in the graph with those edges deleted. The vertices of $R_\zeta$ need not
be connected, and their clusters need not be disjoint. The layout is schematic.}
...
\[
 \PP(o\con b,o\con A)\geq\PP(o\con A,a\con b).
\]
Thus Question~\ref{q9} has an affirmative answer for every minimizer.
\end{theorem}
...
\[
 \sum_{\zeta}\PP(\zeta)\,\EE_\star\bigl[F(C_{R_\zeta})-F(C_a);\ R_\zeta\ncon a,\
 R_\zeta\con A\setminus\{a\}\bigr]\,,
\]
the events and clusters inside the expectation being computed under $\PP_\star$. The
hypothesis on $a$ says $\EE_\star[F(C_a)]\le\EE_\star[F(C_x)]$ for every $x\in A$, so each
...
\]
The phrase \emph{let $a$ minimize} is understood to assert the conclusion for an
...
connected to $b$ in $(V,K)$, and let $g_4(K,L)$ indicate that $o$ is not connected to
$x$ in $(V,L)$. Let $h_1(K,L)$ and $h_2(K,L)$ indicate, respectively, that $o$ is
connected to $x$ and that $x$ is connected to $b$ in $(V,L)$. Then $g_3,g_4$ are
...
$\xi\in\{a\con b,o\ncon b\}$ and a simple open path $P$ from $a$ to $b$ in $\xi$. Let
$\omega=\omega_1\cup P$. This configuration has positive probability,
...
$\omega_1\in\{o\ncon A\}$. Any other such path meets $P$ and would connect $o$ to $b$ in
...
$\{o\con A\}$ holds as well; therefore $\PP(o\con A,a\con b)\le\PP(a\con b)\le\PP(o\con
b,o\con A)$.
...
\[
 s_y=q\rho_a,\qquad
 s_x=q\rho_x+(1-q)\PP(x\con b,o\ncon x)
\]
be the two quantities minimized in the enlarged graph. Since
$\{o\ncon A\}\subseteq\{o\ncon x\}$, we have
$s_x\geq\rho_x=\rho_a>q\rho_a=s_y$. Thus $y$ is the unique minimizer in $\{y,x\}$.
...
\end{equation}
Suppose $\delta=\PP(o\con A,a\con b)-\PP(o\con b,o\con A)$ is positive; then $\delta\le1$,
and \eqref{eq:q8-unique-applied} gives
$q\delta\le(1-q)\PP(o\con b,o\con x)\le1-q$ for every $q$ with $0<q<1$. Taking
$q=1-\delta/4\in[3/4,1)$ turns this into
$(1-\delta/4)\delta\le\delta/4$, that is $\delta\ge3$, a contradiction. Hence $\delta\le0$,
which is \eqref{eq:41}.
\end{proof}

\begin{remark}
It remains open whether \eqref{eq:41} holds for the unique minimizer of $\rho_x$ when
...
Proposition~\ref{prop:q8}. It also checks the strong pre-FKG inequality, the fixed-minimizer
form of Conjecture~\ref{c4}, the stronger form of Conjecture~\ref{c6}, and
...
\nolinkurl{Percolation.Continuity.CSH.phiFun_mono}; and \eqref{eq:markov1} is the
source-set form \nolinkurl{KNAll.Guarded.guardResid_sourceCluster}. The stopped estimate
\eqref{eq:stopped} and its monotonicity use
\nolinkurl{CovTauStarN.yS_mul_mS_le} and
\nolinkurl{CovTau.Yw_antitone_of_le}. Their set-source specialization is checked in
...
Mathlib}, 2026, \url{https://github.com/anthropic-experimental/percolation}.
